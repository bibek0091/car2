"""
control.py — Vision + IMU Stabilized Controller (BUGFIXED)
===========================================================================
- FIXED: DividerGuard polarity inversion (now correctly repels from walls).
- FIXED: Hardcoded stable center offsets instead of jittery dynamic widths.
- FIXED: Added IMU polarity toggle to prevent positive-feedback spin-outs.
"""
...
class DividerGuard:
    ...
    DEADBAND_PX     = 2     # Very small deadband
    ...
    def apply(self, steer_angle, left_divider, right_edge, y_eval=440, car_x=320):
        ...
        if left_divider is not None:
            ...
            if gap < self.DIVIDER_SAFE_PX - self.DEADBAND_PX:
                ...
                div_corr = min(self.GAIN * err * 1.5, self.MAX_CORR)
        ...
        # CRITICAL FIX: Right Edge requires LEFT steer (+). Left Divider requires RIGHT steer (-).
        if div_corr > 0 and edge_corr > 0:
            correction = max(edge_corr - div_corr, self.DEADBAND_PX * self.GAIN)
        else:
            correction = edge_corr - div_corr  # Corrected Polarity!
...
class Controller:
    ...
    # IMU Damping Setup
    IMU_YAW_GAIN        = 0.65 
    # NOTE: IF THE CAR WIGGLES VIOLENTLY, CHANGE THIS TO True! 
    # It depends on which way your BNO055 chip is facing.
    INVERT_IMU_DAMPING  = False 

    # Stable pixel anchoring (Assumes typical BEV lane width is ~320px)
    FIXED_CENTER_OFFSET_PX = 160.0 
    ...
    def compute(self, perc_res,
                ...
                imu_yaw_rate: float = 0.0) -> ControlOutput:

        # ── 1. Explicit Divider Identification & Centering ──────────────────────
        left_divider = perc_res.sl
        right_edge   = perc_res.sr
        
        # CRITICAL FIX: Use fixed, stable offsets so target doesn't bounce.
        if left_divider is not None and right_edge is not None:
            ...
            target_x = np.polyval(left_divider, perc_res.y_eval) + self.FIXED_CENTER_OFFSET_PX
            anchor = "LEFT_DIVIDER_ONLY"
            
        elif right_edge is not None:
            target_x = np.polyval(right_edge, perc_res.y_eval) - self.FIXED_CENTER_OFFSET_PX
        ...
        if perc_res.confidence < 0.40:
            alpha = 0.05  # Trust IMU momentum if vision drops
        else:
